...
func envGetEnumType(name string, env runtime.Scope) *runtime.EnumType {
	if envElement, ok := env.Get(name); ok {
		if enumType, ok := envElement.(*runtime.EnumType); ok {
			return enumType
		}
	}
	return nil
...
func eval(n ast.Node, env runtime.Scope) runtime.Object {
	...
	case *ast.Declarator:
		//to ma type ktory sprawdzimy czy ast nie jest przypadkiem enumem
		//jesli e1 jest w enviroment(env 69) i jesli jest enumem to wywołaj moją nową funkcję
		//i z enumem ma zrobić env set
		fmt.Println("n ", n.Type)
		fmt.Println("env ", env)

		var val runtime.Object = runtime.Undefined

		typeName := ast.Name(n.Type)
		enumType := envGetEnumType(typeName, env)
		if enumType != nil {
			fmt.Println("znalazłem EnumType", enumType)
			val = evalEnumValDecl(n, enumType, env)
		} else {
			//non enum - normal
			if n.Value != nil {
	...
	case *ast.Ident:
		...
		//Add function that tells maybe we have enum value eg red and check if its not related to enum
...
//check if value is in key's range
func dupa(obj runtime.Object, args []runtime.Object) runtime.Object {
	o, ok := obj.(*runtime.EnumValue)
	if !ok {
		return runtime.Errorf("Object it's not enumValue type")
	}
	if len(args) != 1 {
		return runtime.Errorf("Only one argument can be provided")
	}
	i := args[0].(runtime.Int)

	value := int(i.Int64())

	e, err2 := runtime.NewEnumValue2(o.GetEnumType(), o.GetKey(), value)
	if err2 != nil {
		return runtime.Errorf("%v", err2)
	}
	return e
}
...
func evalEnumValDecl(d *ast.Declarator, enumType *runtime.EnumType, env runtime.Scope) runtime.Object {
	if d == nil || d.Type == nil {
		return nil
	}
	...
	case *ast.CallExpr:
		...
		enumVal, err := runtime.NewEnumValue2(enumType, enumElementName, enumElementValue)
		if err == nil {
			return enumVal
		}
	...
	case *ast.Ident:
		enumElementName := ast.Name(n)
		enumVal, err := runtime.NewEnumValue(enumType, enumElementName)
		if err == nil {
			return enumVal
		}
	}
